...
def malicious_train(model, global_model_pre, whole_data_loader, clean_data_loader, poison_data_loader, device, criterion, optimizer,
    ...
    if attack_mode == "none":
        ...
        for  poison_data, clean_data in zip(poison_data_loader, clean_data_loader):
            ...
            print(avg_update_pre)
            print(mine_update_now)
            loss.backward()
            optimizer.step()
        ...
        for  poison_data, clean_data in zip(poison_data_loader, clean_data_loader):
            ...
            loss1 = criterion(output, poison_data[1]) # cross entropy loss
            output = model(clean_data[0])
            loss2 = criterion(output, clean_data[1])


            mine_update_now = parameters_to_vector(list(model.parameters())) - parameters_to_vector(
                list(global_model_pre.parameters()))
            sigma = torch.norm(mine_update_now)
            loss = loss2+loss1+(sigma**2)*torch.randn(1).to(device)
...
class FederatedLearningTrainer(ParameterContainer):
    def __init__(self, arguments=None, *args, **kwargs):
        ...
        if self.defense_method =='layering':
    ...
    def run(self):
        ...
        for flr in range(1, self.fl_round+1):
            ...
            elif self.defense_method =='layering':
                chosens = 'none'
            ...
            if self.defense_method=='layering':
                interval=20
            if self.dataname in ['tiny-imagenet','cifar100']:
                interval=20

            interval=self.fl_round//2
            # interval=200
            if self.defense_method == 'layering':
                ...
                if flr % interval == 0:
                    ...
                    # layer_g=layering_global(self.model_list,benign,self.device)
                    # overall_acc=test_model(layer_g, self.test_data_ori_loader, self.device,print_perform=False)
                    # backdoor_acc=test_model(layer_g, self.test_data_backdoor_loader, self.device,print_perform=False)
                    ...
                    logger.info(out)
                    print(out)
                    logger.info(test_model(self.net_avg, self.test_data_ori_loader, self.device,
                                           print_perform=False))
                    logger.info(test_model(self.net_avg, self.test_data_backdoor_loader, self.device,
                                           print_perform=False))
                    backdoor_acc = 0
                    for i in range(len(benign)):
                        backdoor_acc += out[i]
                ...
                # if flr<50 and self.defense_method=='flame':
                #     interval=1
            ...
            if self.save_model == True:
                ...
                if flr == self.args_lr:
                    torch.save(self.net_avg.state_dict(), "tiny_resnet18.pt")
                    # sys.exit()
        ...
        results_filename = '1-{}_2-{}_3-{}_4-{}_5-{}_6-{}_7-{}_8-{}_9-{}_10-{}_11-{}_12-{}_13-{}_14-{}_15-{}_16-{}' \
                           '_17-{}_18-{}_19-{}_20-{}_21-{}_22-{}_23-{}'.format(
            self.dataname,  #1
