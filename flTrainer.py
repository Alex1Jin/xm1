    ...
    def run(self):
        ...
        for flr in range(1, self.fl_round+1):
            ...
            if self.defense_method =='layering':
                net_list = [copy.deepcopy(self.model_list[i]) for i in range(self.part_nets_per_round)]
            else:
            ...
            if self.defense_method == "none":
                ...
                net_list ,self.net_avg= self.defender.exec(global_model_pre=self.net_avg,client_model=net_list, device=self.device)
                for i in range(len(selected_node_indices)):
                    self.model_list[selected_node_indices[i]]=net_list[i]
            ...
            #
            # self.net_avg = fed_avg_aggregator(net_list, global_model_pre, device=self.device, model=self.model,
            ...
            if self.defense_method=='layering':
                if flr%10==0:
                    ...
                    for i in range(len(net_list)):
                        # print(out[i])
                        backdoor_acc+=out[i]
                        global_user_idx = selected_node_indices[i]
                        if global_user_idx < self.malicious_ratio * self.num_nets:
                            backdoor_acc-=out[i]
                            mn+=1
                ...
                logger.info("############ Averaged Model : Norm {}".format(torch.norm(v)))
                if flr%10==0:
                    logger.info("Measuring the accuracy of the a    veraged global model, FL round: {} ...".format(flr))
